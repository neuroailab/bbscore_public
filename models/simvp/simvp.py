...
class SimVP:
    """Loads pre-trained SimVP video model (trained on KITTI)."""

    def __init__(self):
        """Initializes the video model loader."""
        self.model_mappings = {
            "SimVP": {"model": "https://github.com/chengtan9907/OpenSTL/releases/download/kitti-weights/kitticaltech_simvp_gsta_one_ep100.pth",
                      "config": "models/simvp/simvp_config.py"
                      }
        }
    ...
    def _get_config(self, identifier, config_path):
        # Load config from local file path
        if not os.path.exists(config_path):
            # If running from a different root, might need adjustment,
            # but we assume relative path from execution root based on prompt.
    ...
    def get_model(self, identifier):
        ...
        # Overrides for consistency with weights
        config["method"] = config["method"]
        config['dataname'] = "kitticaltech"
        config['metrics'] = ['mse', 'mae']
        config['in_shape'] = [10, 3, *IMAGE_SIZES]  # [T, C, H, W]
    ...
    def postprocess_fn(self, features_np):
        ...
        """
        # features_np shape depends on layer, usually [B, T, C, H, W] or [B, C, T, H, W]
        # We flatten everything after Batch dim.
