...
class TorchWoodburyMetric(RidgeAutoMetric):
    def __init__(
        self,
        ceiling: Optional[float] = None,
    ):
        super().__init__(ceiling=ceiling, mode='woodbury')


class TorchBlockMetric(RidgeAutoMetric):
    def __init__(
        self,
        ceiling: Optional[float] = None,
    ):
        super().__init__(ceiling=ceiling, mode='block')


class RidgeChunkedMetric(RidgeMetric):
    def __init__(
        self,
        ceiling: Optional[float] = None,
        chunk_size=4000,
    ...
    def compute_raw(
        ...
        # if we have a trial axis, flatten it
        if target.ndim == 3:
            return self._compute_raw_3d(
                source,
                target,
                test_source,
                test_target,
        ...
        # Now run your chunked cross-validation with the fixed alpha array.
        if test_source is None:
            return run_kfold_cv_chunked(model_factory, source, target, scoring_funcs, self.chunk_size, stratify_on=stratify_on)
        return run_eval_chunked(model_factory, source, target, test_source, test_target, scoring_funcs, self.chunk_size)
    ...
    def _compute_raw_3d(
        ...
        """
        Helper for 3D targets (N, M, T): flattens M×T, runs CV or eval, then
        reshapes and averages over the trial axis.
        """
        N, n_reps, n_voxels = target.shape
        ...
        if test_source is None:
            ...
            # ── Expand the 2D eval into per-rep loops, using run_eval_chunked to do
            #    all the chunked‐voxel work, then average over test reps ──
            raw_flat: Dict[str, List[np.ndarray]] = {
                name: [] for name in scoring_funcs}

            # for each training repetition…
            for train_rep in tqdm(range(n_reps), desc="Train reps"):
                # shape (N, n_voxels)
                y_train_rep = target[:, train_rep, :]

                # collect this train_rep’s scores on each test trial
                rep_scores: Dict[str, List[np.ndarray]] = {
                    name: [] for name in scoring_funcs
                }

                for test_rep in tqdm(range(n_reps_test), desc="Test reps", leave=False):
                    # shape (N_test, n_voxels)
                    y_val_rep = test_target[:, test_rep, :]

                    # chunked eval on this train–test pair
                    eval_scores = run_eval_chunked(
                        model_factory,
                        source,
                        y_train_rep,
                        test_source,
                    ...
                    for name, fold_arr in eval_scores.items():
                        # fold_arr.shape == (n_folds, n_voxels); average over folds
                        rep_scores[name].append(fold_arr.mean(axis=0))

                # now average across the test-trial axis → one vector of length n_voxels
                for name, arr_list in rep_scores.items():
                    # stack into (n_reps_test, n_voxels) then mean→(n_voxels,)
                    mean_over_tests = np.stack(arr_list, axis=0).mean(axis=0)
                    raw_flat[name].append(mean_over_tests)

            # flatten each name’s list (n_reps × n_voxels → n_reps*n_voxels)
            raw_flat = {name: np.concatenate(v, axis=0)
                        for name, v in raw_flat.items()}

        # Reshape back to (M, T) and average over T
        raw: Dict[str, np.ndarray] = {}
        for name, scores_flat in raw_flat.items():
            # if eval‐mode, scores_flat has length n_reps*n_voxels:
            scores = scores_flat.reshape(
                n_reps, n_voxels) if test_source is not None else scores_flat.reshape(10, n_reps, n_voxels)
            raw[name] = scores.mean(axis=1)   # one score per train repetition
            # full per-voxel map
            raw['trial_based_raw_'+name] = scores
