...
class PhysionDetection(BaseDataset):

    BUCKET = "gs://bbscore_datasets/PhysionDetection"
    ...
    def __init__(
        ...
        self.video_folder_name = "stimulus_PhysionGlobalFocusedLongDetection2024"
        self.video_path = os.path.join(
            self.root_dir, self.video_path_base, self.video_folder_name)
        self._prepare_videos()
        print(
            f"PhysionDetection '{self.split_name()}' initialized. RandAugment: {self.apply_randaugment and self.train}.")
    ...
    def _download_physion_data(self):  # Ensure shutil is imported
        ...
        csv_fname = "stimulus_PhysionGlobalFocusedLongDetection2024.csv"
        zip_fname = "stimulus_PhysionGlobalFocusedLongDetection2024.zip"

        behavioral_data_fname_key = "PhysionHumanContactDetection2024.nc" if not self.placement else "PhysionHumanPlacementDetection2024.nc"
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Union[float, Tuple[str, float, float]], int]:
        ...
        if self.preprocess is None:
            raise ValueError(
                "preprocess is not set for PhysionDetection dataset.")
    ...
    def _prepare_videos(self):
        ...
        if not self.train:
            ds = xr.open_dataset(self.local_behavioral_nc)
        ...
        csv_path = os.path.join(
            self.root_dir, "stimulus_PhysionGlobalFocusedLongDetection2024.csv")
        with open(csv_path, newline="") as fin:
            ...
            for row in reader:
                ...
                stim_id_base = row["stimulus_id"][:-4]  # remove _sim_XXX
    ...
    def _load_data(self, video_path: str) -> List[Image.Image]:
        ...
        if frames.shape[0] > int(self.target_fps * 4):
            frames = frames[:self.target_fps * 4]
        frames = [Image.fromarray(frame.numpy()) for frame in frames]
        while len(frames) < int(self.target_fps * 4):
            frames += [frames[-1]]
        return frames


class PhysionContactDetectionTrain(PhysionDetection):
    def __init__(self,
                 root_dir: Optional[str] = None,
                 overwrite: bool = False,
                 preprocess: Optional[Callable] = None,
                 ):
        root = os.path.join(get_data_home(), PhysionDetection.__name__)
...
class PhysionContactDetectionAugmentedTrain(PhysionDetection):
    def __init__(self,
                 root_dir: Optional[str] = None,
                 overwrite: bool = False,
                 preprocess: Optional[Callable] = None,
...
class PhysionContactDetectionTest(PhysionDetection):
    """Dataset class for the testing set of the Physion Contact Detection Stimulus."""
    ...
    def __init__(
        ...
        root = os.path.join(
            get_data_home(), PhysionDetection.__name__
        )
        super().__init__(root_dir=root, overwrite=overwrite, preprocess=preprocess,
                         train=False, placement=False, intrageneralization=False)


class PhysionIntraContactDetectionTrain(PhysionDetection):
    """Dataset class for the training set of the Physion Contact Detection Stimulus for the Intra-configuration generalization Task."""
...
class PhysionIntraContactDetectionAugmentedTrain(PhysionDetection):
    """Dataset class for the training set of the Physion Contact Detection Stimulus for the Intra-configuration generalization Task."""
...
class PhysionIntraContactDetectionTest(PhysionDetection):
    """Dataset class for the testing set of the Physion Contact Detection Stimulus for the Intra-configuration generalization Task."""
...
class PhysionPlacementDetectionTrain(PhysionDetection):
    """Dataset class for the training set of the Physion Placement Detection Stimulus."""
...
class PhysionPlacementDetectionAugmentedTrain(PhysionDetection):
    """Dataset class for the training set of the Physion Placement Detection Stimulus."""
...
class PhysionPlacementDetectionTest(PhysionDetection):
    """Dataset class for the testing set of the Physion Placement Detection Stimulus."""
...
class PhysionIntraPlacementDetectionTrain(PhysionDetection):
    """Dataset class for the training set of the Physion Placement Detection Stimulus for the Intra-configuration generalization Task."""
...
class PhysionIntraPlacementDetectionAugmentedTrain(PhysionDetection):
    """Dataset class for the training set of the Physion Placement Detection Stimulus for the Intra-configuration generalization Task."""
...
class PhysionIntraPlacementDetectionTest(PhysionDetection):
    """Dataset class for the testing set of the Physion Placement Detection Stimulus for the Intra-configuration generalization Task."""
